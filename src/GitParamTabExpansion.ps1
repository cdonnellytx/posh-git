...
$shortGitParams = @{
    ...
    rm = 'f n r q'
    shortlog = 'n s e w'
    show = 'p u s U= t X X= z B B= M M= C C= D l= S= G= O= R a b w I= W'
    stash = 'p k u a q'
    status = 's b u z'
    submodule = 'q b f n N'
...
$longGitParams = @{
    ...
    rebase = 'onto keep-base continue abort quit apply empty= no-keep-empty keep-empty reapply-cherry-picks no-reapply-cherry-picks allow-empty-message skip edit-todo show-current-patch merge strategy= strategy-option= rerere-autoupdate no-rerere-autoupdate gpg-sign no-gpg-sign quiet verbose stat no-stat no-verify verify no-ff force-rebase fork-point no-fork-point ignore-whitespace whitespace= committer-date-is-author-date ignore-date reset-author-date signoff interactive rebase-merges preserve-merges exec root autosquash no-autosquash autostash no-autostash reschedule-failed-exec no-reschedule-failed-exec update-refs no-update-refs preserve-merges'
    ...
    show = 'pretty pretty= format= abbrev-commit no-abbrev-commit oneline encoding= expand-tabs= expand-tabs no-expand-tabs notes notes= no-notes show-notes show-notes= no-standard-notes standard-notes show-signature patch no-patch diff-merges= no-diff-merges combined-all-paths unified= output= output-indicator-new= output-indicator-old= output-indicator-context= raw patch-with-raw indent-heuristic no-indent-heuristic minimal patience histogram anchored= diff-algorithm= stat stat= compact-summary numstat shortstat dirstat dirstat= cumulative dirstat-by-file dirstat-by-file= summary patch-with-stat name-only name-status submodule submodule= color color= no-color color-moved color-moved= no-color-moved color-moved-ws= no-color-moved-ws word-diff word-diff= word-diff-regex= color-words color-words= no-renames no-rename-empty rename-empty check ws-error-highlight= full-index binary abbrev abbrev= break-rewrites break-rewrites= find-renames find-renames= find-copies find-copies= find-copies-harder irreversible-delete diff-filter= find-object= pickaxe-all pickaxe-regex skip-to= rotate-to= relative relative= no-relative text ignore-cr-at-eol ignore-space-at-eol ignore-space-change ignore-all-space ignore-blank-lines ignore-matching-lines= inter-hunk-context= function-context ext-diff no-ext-diff textconv no-textconv ignore-submodules ignore-submodules= src-prefix= dst-prefix= no-prefix default-prefix line-prefix= ita-invisible-in-index'
